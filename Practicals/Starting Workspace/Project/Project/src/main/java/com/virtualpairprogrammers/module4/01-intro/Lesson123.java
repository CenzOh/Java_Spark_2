 ...
 * 
 * With the example code we are given we could either make a new project or make a new package called .streaming and put loggin server file in that. Now note in advanced
 * this is a bad implementation of socket server since when we connect to this socket server we need to disconnect, then stop, restart server since as things stand, this socket server
 * has no way of knowing if a client has disconnected as it is tough to do. We want something simple to run. THis program randomly generates server log messages and we will 
 * continuously read the messages. It has random date adn time, outputs to network socket. Simulation of stream of messages. We can connect to this socket easily in Spark streaming.
 * Not prod standard but a good into.
 */
}
