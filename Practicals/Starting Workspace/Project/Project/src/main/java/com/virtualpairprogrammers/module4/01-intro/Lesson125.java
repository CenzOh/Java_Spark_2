 ...
 */
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Logger.getLogger("org.apache.spark.storage").setLevel(Level.ERROR); //removes the annoying warning messages
 ...
 * One thing you will notice is at the end of each batch, results are presented to us as a timestamp for each batch. We can see if it worked we have a lot of logging messages.
 * 
 * -------------------------------------------
 ...
 * Lets remove that annoying warning stream of messages, comes from spark.storage
 * 
 * Logger.getLogger("org.apache.spark.storage").setLevel(Level.ERROR);
 * 
 * Note, now that we made a change to the spark code we MUST terminate the log server again. If you dont, our program will NOT recieve any logigng messages. So termiante both.
 * Again not production standard but this is fine for now.
 */
