 ...
 * 
 * Typical architecture, typical stream of events. Can be from anywhere like clicks on websites. We can catch these in kafka, it can keep a historic record of these events. And it connects
 * consumers to kafka. Consumer is spark streaming that receives event batches to perform analysis. We can connect other consumers to the broker like a micro service.
 * Kafka is also popular because it is implemented so elegantly. Using a PSS can be a nightmare since there's a lot involved to keep track of subscribers.
 * 
 * Events -> Kafka |Events| -> Spark Streaming
 * 							- --> Microservice
 */
}
