...
//COME BACK TO THE FOR LOOP LATER
		...
		for (int noOfClusters = 2; noOfClusters <= 8; noOfClusters++) { //WILL COME BACK TO THIS
			...
			kMeans.setK(noOfClusters); //changes num of clusters
			System.out.println("No of clusters " + noOfClusters); //prints num of clusters
 ...
 */
			...
			for (Vector v : clusterCenters) { System.out.println(v); } //when we do the loop, we can comment these tweo lines out
 ...
 * By the way we dont have to print out the clusters for each one. We would want to see how many are in each model tho.
 * 
 * Lets just break down results. 
 * When clusters was 2, SSE was 1439 and silhouette was 0.67. 
 * 4 clusters is silhouette of 0.48, SSE is 1136.
 * 
 * If we can demonstrate this in a graph, that would show the best interpretations. For SSE, the more clusters, the lower the SSE value goes. Theres a break point between 5 and 6. 
 * So the sum of squared decreases a lot less at each cluster increase. So we can see that 5 is the elbow in the graph. THats what we are looking for. 
 * In Silhouette, when we get to 5 clusters, theres a slight increase from 4, but doesnt increase very much after that. Kind of goes down.
 * So 5 is our best bet right now. A bit of guesswork to try to put the graphs together.
 * 
 * If we view what happens with clusters at 5, theres a reasonable spread. Cluster 3 has 2 people for instance. SSE minimzed to 612 and silhouette is 0.47.
 * Now we can look at each competitor in each group and find best performing one. We can find avgs of cluster centers, understand why different competitors would be put in different groups.
 * Can be used as a recommender system. We will look at another unsupervised model that is more suited as a recommender system. 
 */
