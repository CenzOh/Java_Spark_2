 ...
 */
			...
			model2.setColdStartStrategy("drop"); //WILL EXPLAIN THIS IN A FEW LINES
 ...
 * what we've already seen. So now that we updated it:
 * 
 * model2.setColdStartStrategy("drop");
 * 
 * We will see the output data set will not include any records where we cant make a prediction. So the first table that we saw from the transform will be empty since we cant make a prediction.
 */
		}
	}
}