...
import static org.apache.spark.sql.functions.*; //for UDF importing all static functions
 ...
 * 
 * Lets use the UDF to change it into a group. Lets register the UDF in spark session. Write the name, the fcn we defined, return type which is of type String
 */
		spark.udf().register("countryGrouping", countryGrouping, DataTypes.StringType); 
		
/* manipulate our data set BEFORE calling show. Replace our country column. Use .withColumn(). Give it same name of country. Replace it with UDF. use the callUDF() funciton, refer to UDF
 * name of countryGrouping. Input will be current row in the country col. So use .col() method. Do static import fo col method
 */
